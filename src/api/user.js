      ...
      .then(body => {
        if (body.startsWith(config.grabdata.errSystemErrorText)) {
          ...
          let [
            ...
            sessionToken,
            username
          ] = /^<meta http-equiv="refresh" content=0;url=select_entry\.php\?ACIXSTORE=(.*?)&hint=(.*?)>$/.exec(
            body
          )

          correctRequest({
            url: config.grabdata.selectPage
...
export function logout(sessionToken) {
  return new Promise((resolve, reject) => {
    request({
      url: config.grabdata.logoutPage.replace('{0}', sessionToken),
      method: 'POST'
    })
      .catch(() => {
        // Default to 302
